...
User.queries.add_friend =       "INSERT INTO `Friends` (`user_id`, `friend_id`, `added`) VALUES(:user_id, :friend_id, :added)";
User.queries.delete_friend =    "DELETE FROM `Friends` WHERE `Friends`.`user_id` = :user_id AND `Friends`.`friend_id` = :friend_id LIMIT 1";

/* error codes */
User.error.DB_ERROR 		= 1;
User.error.EXIST 			= 2;
User.error.FAILED_LOGIN 	= 3;
User.error.VALIDATION 		= 4;
...
User.instance.prototype.addFriend = function(friend_id) {
    var self = this;
    new Query(User.queries.add_friend, {
...
User.instance.prototype.deleteFriend = function(friend_id) {
    var self = this;
    new Query(User.queries.delete_friend, {
        user_id: this.user.id,
        friend_id: friend_id
    }).
    execute(function(err, rows) {
...
User.instance.prototype.generateUpdateQuery = function(id, params) {
    var q = "UPDATE `Users` SET ";
    for(var i in params) {
        q += "`Users`.`"+ i +"` = '"+ params[i] +"', ";
    }

    return q.substring(0, q.length - 2) + " WHERE `Users`.`id` = " + id;
}
User.instance.prototype.update = function(params, cb) {

    var q = this.generateUpdateQuery(this.user.id, params);
    new Query(q).
    execute(function(err, rows) {
        if(err) {
            var e = defaults.get("error");
...
User.instance.prototype.fetch = function(cb) {
	var self = this;
