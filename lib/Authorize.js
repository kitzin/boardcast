...
Authorize.level.session = {
	...
	cb: function(req, res, next) {
		...
		if(h.body.isset("session_token") === false || h.session.validToken() === false) {
			// Send error
			res.redirect("/?error=token"); // for now
		}
		else {
			next();
		}
...
Authorize.level.login = {
	...
	cb: function(req, res, next) {
		...
		if(!h.session.isLogin()) {
			// Send error 
			res.redirect("/login"); // for now if ajax sent back ajax object
